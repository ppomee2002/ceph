#!/bin/bash
#
# Ceph 벡터 LSH 벤치마크 (PG map mode: mixed)
# - 기존 run_vector_bench.sh를 유지하고
# - --pg-map-mode mixed 옵션으로 별도 실험
#

LOG_FILE="benchmark_results_mixed.txt"
PG_NUM=1024
NUM_TABLES=32
QUERY_NUM=100
PROBE_MODE="vote"
...
main() {
  ...
  for W in "${WRITE_TOP_PGS_ARRAY[@]}"; do
    POOL_NAME="vector_pool_${PG_MAP_MODE}_t${NUM_TABLES}_w${W}"
    log "========================================"
    ...
    log "[2단계] Load 실행: num-tables=${NUM_TABLES}, write-top-pgs=${W}, pg-map-mode=${PG_MAP_MODE}"
    if ! $BENCH_BIN --file "$BASE_FVECS" --pool "$POOL_NAME" \
      --num-tables "$NUM_TABLES" --write-top-pgs "$W" --pg-num "$PG_NUM" \
      --pg-map-mode "$PG_MAP_MODE" \
